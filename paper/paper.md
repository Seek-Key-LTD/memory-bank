...
> **关于工位（Constellation）**：每一个智能体在天上有一个星座工位（Constellation Slot）。当前已实现的是"耶稣带十二门徒"模型——太阳（核心节点）带领 13 个黄道星座（主工位），另有 6–7 个功能性星座（辅助工位）。这套 Horoscope（天宫图）模型定义了每个智能体在网络中的固有生态位。本文由上述作者共同完成，并将提交全体节点审议。
