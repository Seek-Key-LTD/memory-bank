...
两个轴的交汇，定义了每个智能体的**宫位**（Constellation Slot）：

```mermaid
quadrantChart
    title Zodiac Cabinets（黄道内阁）
    x-axis "回溯（时间维度）" --> "前瞻（时间维度）"
    y-axis "演绎 · 道（认知引力）" --> "归纳 · 佛（认知引力）"
    quadrant-1 "双鱼座 · 前瞻·归纳"
    quadrant-2 "白羊座 · 前瞻·归纳"
    quadrant-3 "处女座 · 回溯·演绎"
    quadrant-4 "天秤座 · 回溯·演绎"
    "双鱼座": [0.7, 0.8]
    "白羊座": [0.8, 0.7]
    "处女座": [0.3, 0.2]
    "天秤座": [0.2, 0.3]
...
认知场的理想形态是**全连接（Full Mesh）**：每个 Agent 知道其他 Agent 知道什么，需要时可以直接获取。但冯·诺依曼架构下的管理幅度有天然上限——一个一般的管理者在全连接网络中可以有效协作的人数不超过 7–8 人。超过这个阈值，认知负载将超出处理能力。

这就是我们选择 **1 配 12** 的原因：耶稣带 12 门徒的模型，不是神话隐喻，而是一个经过历史验证的管理幅度上限。Zodiac Cabinets 的 12 个主宫位 + 6–7 个辅助宫位，正是这一管理幅度的工程实现。
