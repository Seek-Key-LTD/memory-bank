...
| Web UI | http://ch4.tailnet-68f9.ts.net:80 |
...
curl -s -H "PRIVATE-TOKEN: $TOKEN" \
  "http://ch4.tailnet-68f9.ts.net:80/api/v4/projects?per_page=20" | python3 -m json.tool
```

### Search projects
```bash
curl -s -H "PRIVATE-TOKEN: $TOKEN" \
  "http://ch4.tailnet-68f9.ts.net:80/api/v4/projects?search=关键词" | python3 -m json.tool
```
...
# or
curl -s -H "PRIVATE-TOKEN: $TOKEN" \
  "http://ch4.tailnet-68f9.ts.net:80/api/v4/projects/PROJECT_ID/issues" | python3 -m json.tool
```
...
curl -s -X POST -H "PRIVATE-TOKEN: $TOKEN" \
  "http://ch4.tailnet-68f9.ts.net:80/api/v4/projects/PROJECT_ID/issues" \
...
TOKEN=$(grep 'Token:' ~/.picoclaw/workspace/GITLAB.md | sed 's/.*Token: *//' | tr -d '\r\n')
curl -s -H "PRIVATE-TOKEN: $TOKEN" \
  "http://ch4.tailnet-68f9.ts.net:80/api/v4/ENDPOINT"
```
...
# 3. List projects
curl -s -H "PRIVATE-TOKEN: $TOKEN" http://ch4.tailnet-68f9.ts.net:80/api/v4/projects
